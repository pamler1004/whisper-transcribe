...
5. **快速校对并报告结果**：转录后立刻进入下方的「快速校对」；完成后列出输出 md/srt 的绝对路径

### 快速校对（转录后的必经步骤）

转录完成后，先发一条简短进度：`[2/2] 转录完成，正在快速校对明显识别错误…`，然后通读全文一次并直接覆盖输出文件。

**只修正能由上下文明确确认的错误**：同音/近音错字、明显错词或断词、中英混排中的产品名/品牌名、明显错误的数字和单位。

**保持转录忠实**：不改写语气，不删除口头语或重复，不总结，不调整句子/段落顺序；无法从文字上下文确定的内容不猜、不改。MD 必须维持原有行数和行序；SRT 只能修改 cue 文本，绝不修改 cue 序号或时间戳。
...
- **推荐每批最多 10 个文件或累计 3 小时音频**（以先到者为准）。M1 Pro 16GB 上 large-v3 实测约 4.7x 实时，3 小时音频转录约 38 分钟；更多文件应拆批。
