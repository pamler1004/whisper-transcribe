...
2. **在转录前询问格式与是否校对**：文件确认存在后、**执行转录脚本前**，必须先问用户两件事，不得默认。Claude Code 用 `AskUserQuestion` 工具一次问两题（JSON 见下）；Codex 或其他 agent 直接用文本提问并等用户回答。
   - 导出格式：只要 MD / SRT + MD
   - 是否校对：要快速校对 / 不用校对（只要原始稿，最快）
     ...
     "questions": [
       {
         ...
         "options": [
           {"label": "只要 MD", "description": "逐句分行的 Markdown 逐字稿，适合阅读、整理口播稿和转笔记。"},
           {"label": "SRT + MD", "description": "同时输出带时间戳字幕（SRT）和逐句分行逐字稿（MD），适合后续配字幕视频。"}
         ]
       },
       {
         "header": "是否校对",
         "multiSelect": false,
         "question": "转录后要不要快速校对明显的识别错误？",
         "options": [
           {"label": "要校对", "description": "通读原始稿，只修同音错字、专名、断词，另存为「-校对版」，原始稿保留。"},
           {"label": "不用校对", "description": "只输出 ASR 原始稿，最快，不做任何修改。"}
         ]
       }
     ]
   }
...
5. **按校对选择收尾**：
   - 选「要校对」：转录后进入下方「快速校对」，校对版写入「-校对版」文件（原始稿保留），最后列出原始稿与校对版的绝对路径
   - 选「不用校对」：只输出原始稿、不做校对，直接列出原始稿的绝对路径
...
- **单文件**：只发必要阶段提示。选校对：`[1/2] 正在转录：文件名` → `[2/2] 转录完成，正在快速校对明显识别错误…`；不校对：`[1/1] 正在转录：文件名` → 完成后列路径。
- **批量**：顺序处理，每个文件开始/完成各报告一次：`[n/总数] 正在转录：文件名`、`[n/总数] 已完成：文件名`。选校对则每个文件转录后立即校对、落盘再下一个；不校对则转录完直接落盘。单个失败不影响已完成文件。
